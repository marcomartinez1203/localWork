...
const App = {
  ...
  updateAuthUI() {
    ...
    if (isAuth && user) {
      ...
      const modeSwitchText = preferredMode === 'services'
        ? 'Cambiar a modo empleo'
        : 'Cambiar a modo servicios';
      ...
      const avatarHtml = `
        ...
        <div>
          ...
          <div class="user-dropdown" id="userDropdown">
            ...
            <button class="user-dropdown__item" onclick="App.switchPreferredMode()"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:text-bottom;margin-right:8px;"><path d="M3 10.5h14.25m0 0-3.75-3.75m3.75 3.75-3.75 3.75M21 13.5H6.75m0 0 3.75-3.75M6.75 13.5l3.75 3.75"/></svg> ${modeSwitchText}</button>
            <div class="user-dropdown__divider"></div>
  ...
  async switchPreferredMode() {
    const user = AuthService.getUser();
    if (!user) return;
    const currentMode = user.preferred_mode === 'services' ? 'services' : 'jobs';
    const nextMode = currentMode === 'services' ? 'jobs' : 'services';
    const nextUser = { ...user, preferred_mode: nextMode };
    sessionStorage.setItem('lw_user', JSON.stringify(nextUser));
    try {
      await AuthService.updateProfile({ preferred_mode: nextMode });
    } catch (e) { /* backend may ignore this field */ }
    const target = nextMode === 'services'
      ? this._pagePrefix() + 'workers.html'
      : (user.role === 'employer'
        ? this._pagePrefix() + 'dashboard.html'
        : this._pagePrefix() + 'home.html');
  ...
  _navigateTo(url) {
    // Prevent double navigation clicks and make transitions feel smoother.
    ...
    sessionStorage.setItem('lw_nav_transition', '1');
  ...
  _applyEntryTransition() {
    if (!document.body) return;
    const shouldAnimate = sessionStorage.getItem('lw_nav_transition') === '1';
    if (!shouldAnimate) return;
    sessionStorage.removeItem('lw_nav_transition');
